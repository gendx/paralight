...
/// A helper struct for the implementation of [`VecParallelSource`], that wraps
/// a [`*const T`](pointer). This enables sending `T` to other threads.
...
/// [`VecParallelSource::descriptor`]). Therefore we make it [`Sync`] if and
/// only if `T` is [`Send`].
